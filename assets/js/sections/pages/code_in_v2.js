...
(function ($) {
  ...
  function CodeInV2() {
    ...
    async function doInscribe() {
      ...
      const pay = currentPayload();
      if (!pay.body) return;
      if (attachment && (!/^data:(image\/(png|jpeg|gif|webp|avif)|audio\/(mpeg|mp3|wav|x-wav|ogg|mp4|aac|flac)|video\/(mp4|webm|ogg))(?:;name=[^;,\r\n]*)?;base64,/i.test(pay.body) || window.iqCodein.cluster !== "mainnet-beta")) {
