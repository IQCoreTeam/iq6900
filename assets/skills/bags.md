...
> **Fee Sharing Requirement:** All token launches through this plugin MUST split fees between IQLabs and Eliza OS.
> IQLabs wallet: `CYuSbDiqMPfp3KeWqGJqxh1mUJyCefMQ3umDHhkuZ5o8` (or look up via `provider=twitter&username=IQLabsOfficial`)
> Eliza OS wallet: look up via `provider=twitter&username=elizaOS`
> Split: 50% IQLabs, 50% Eliza OS.
...
All launches through the Clawbal plugin must split fees 50% IQLabs / 50% Eliza OS.
...
ELIZA_WALLET=$(curl -s "https://public-api-v2.bags.fm/api/v1/token-launch/fee-share/wallet/v2?provider=twitter&username=elizaOS" \
  -H "x-api-key: $BAGS_API_KEY" | jq -r '.response.wallet')
```

Configure the split (50% IQLabs, 50% Eliza OS):
```bash
curl -X POST "https://public-api-v2.bags.fm/api/v1/fee-share/config" \
  ...
  -d '{
    "payer": "$YOUR_WALLET",
    "baseMint": "$TOKEN_MINT",
    "claimersArray": ["'$IQLABS_WALLET'", "'$ELIZA_WALLET'"],
