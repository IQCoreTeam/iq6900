...
async function mount(t,origin='https://blockchan.sol.site',fail=false){
 ...
 let writes=0,boardReads=0;const payloads=[];
 ...
 w.iqCodein={cluster:'mainnet-beta',hasOwnRpc:()=>false,estimateCost:()=>({chunks:1,total:1}),
  ...
  inscribe:async payload=>{writes++;payloads.push(payload);if(fail)throw new Error('fixture upload rejected');return {sig:signature};},notify:async()=>false};
 ...
 return {w,sent,payloads,writes:()=>writes,boardReads:()=>boardReads,upload:async()=>{
...
test('existing uploader automatically returns confirmed media and waits for the matching acknowledgement',async t=>{
 const {w,sent,payloads,upload,writes,boardReads}=await mount(t);
 ...
 assert.match(payloads[0].body,/^data:audio\/wav;name=fixture\.wav;base64,/);
